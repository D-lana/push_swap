...
/*   Updated: 2021/10/01 17:52:11 by dlana            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "push_swap.h"

static int	ft_check_result(t_point *points, int *array)
{
	int		i;
	t_list	*tmp;
...
static void	ft_check_duplicate(t_point *points, int *array)
{
	...
	while (count != points->count_a)
	{
		if (count > 0 && array[count] == array[count - 1])
			ft_error();
		if (count < points->count_a && array[count] == array[count + 1])
			ft_error();
		count++;
	}
}
...
void	ft_sort_array_to_check(t_point *points)
{
	...
	*array = *ft_quick_sort(array, 0, (count - 1), (count - 1));
	...
	result = ft_check_result(points, array);
	if (result == 0)
		write(2, "OK\n", 3);
	else
		write(2, "KO\n", 3);
